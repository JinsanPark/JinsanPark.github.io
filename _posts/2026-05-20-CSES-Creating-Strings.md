...
bacaa
bcaaa
여기서 b랑 a랑 자리 바꾸면 딱 20개 나올거 같은데.
aaacb
aacab
acaab
caaab
...
규칙이 보일듯 말듯하네. 도저히 안보여서 ai 물어보니 같이있는것 순열 라고함.
총 5개이고 중복이 있는 개수의 팩토리얼 만큼 나눠주면 된다.
...
이제 문제인게, 어떻게 중복없는 배열을 만들것이냐 인데, 이것도 한참 헤매다가 도움을 요청함.

> **힌트 1**
> 이 문제는 가짓수 개수만 묻나요, 아니면 가능한 경우를 하나하나 다 만들어 출력해야 하나요? 후자라면 어떤 풀이 방식이 떠오르나요? 그리고 "첫 칸을 정하고 나면 남은 부분은 무슨 문제"가 되는지 말로 표현해보세요.
{: .prompt-tip }

- 가능한 경우를 하나하나 다 만들어 출력해야하는 문제인데, 여기에 알맞는 알고리즘이 딱히 떠오르지 않음. 브루트포스? 
- 브루트포스는 좀 더 큰 틀이고, 이 문제 같은 경우에는 재귀와 백트래킹으로 푼다고 함.
...
- 들어갈때 alpCount[i]--; (알파벳 하나 빼서 붙히니깐) 다시 돌아올때 alpCount ++. 가장 헷갈렸던 부분이 어떻게 중복된걸 다시 안쓰는지 였는데, alpCount에서 인덱스가 먼저 돌아가고, 그 다음에 반복문 i가 증가하니깐, 이중 반복문 처럼 alpCount++ --가 먼저 돌아가서 중복없는 문자열이 생성됨.
...
그래서 2차 제출에서는 함수의 결과만 저장하는 StringBuilder를 또 하나 생성하여 값을 그곳에 저장한후, 함수가 끝난후 그 값들만 따로 출력함. 이에 따라 시간이 0.33초에서 0.13초로 줄어듬.

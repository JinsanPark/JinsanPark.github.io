...
처음에는 스택을 사용할까 생각함.
맨 왼쪽에 1부터 쭉 넣고, 하나씩 뽑아서 다른 막대기에 넣으며 아래거 보다 위에 넣을 값이 작으면 넣기를 반복 하면서 스택 만들어서, 다 차면 출력하고 다음으로 넘어가는 방식으로.

<details markdown="1">
<summary><b>하노이탑 계산 노가다 (클릭하여 펼치기)</b></summary>
...
그런데 문제가 옮기는 걸 어떻게 카운팅 하고, 어떻게 옮긴 횟수가 최소인지 판단할지, 이 부분을 스택으로 해결하자니 너무 복잡하다고 느껴졌음. 코드도 항상 내 아래에 있는 수가 위에 있는 수 보다 작니? 라고 if문을 사용해서 계속 확인해야할거 같은데, 이러면 너무 느릴거 같다고 생각함.
...
> **힌트 3**
> hanoi(?, ?, ?, n-1)의 세 인자 자리에, 지금 함수가 받은 depart/via/arrive 중 무엇이 어디에 들어가야 할까요? 특히 "보조 기둥" 자리에는 뭐가 들어갈지 짚어보세요.
{: .prompt-tip }

- hanoi(왼, 오, 중, n -1 ) 이거는 왼쪽에 있는 수를 맨 오른쪽 기둥을 통해서, 중앙으로 옮긴다.(중, 왼, 오, n-1) 이거는 중앙에 있는 수를 왼쪽을 통해서 오른쪽으로 옮긴다.
그리고 이 처음 호출하는 hanoi(); 요 함수의 인자값을 어떻게 줘야할지 모르겠어서 다시 힌트 요청함.
...
- 호출하는 hanoi()에 1,2,3,n 이 들어가면 된다고 판단함. 기둥에서 옮기고, 어디서 어디로 옮겼는지 코드에서 출력하면 되니깐.

...
기존거랑 비교해서 하나만 다르다. - 이런게 하면 전체 문자열을 비교하는거라 너무 오래걸릴거 같아다고 생각은 했는데 다른 방법을 찾지 못해 AI에게 힌트를 요청함.
...
{: .prompt-tip }
- 힌트를 보고, 순서를 바꿔서 문자열의 길이 부터 시작해서, n까지 0을 먼저 채운후에, 나중에 sb에 넣으면 되겠다고 생각해 코드를 변경함.

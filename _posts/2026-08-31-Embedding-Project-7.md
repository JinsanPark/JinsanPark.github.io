...
### 2. 캐시는 눈으로 안 보임.
그리고, 저 프로젝트 같은 경우는, 캐시가 눈에 보이지 않습니다.<br>
...
허나 이번에는 조금 달랐습니다. 처음에는 도통 뭘 검증해야하는지 감이 안왔습니다.
...
그러면 제 코드에서는 테스트를 몇개를 만들어야 할까요?
getVector는 목적이 2개죠. if문에서 캐시 히트, 캐시 미스를 검증하죠.
즉, 테스트 코드도 2개입니다. 뭘 검증해야할지 감이 안왔는데, 머리를 싸맬게 아니라, 답은 코드 안에 있었습니다.

들어가기 전에, 간단하게 문법부터 살피고 가봅시다.
```java
mock() // 가짜 일꾼
...
가짜를 왜 쓰냐하면, API를 쓰면 작지만 돈이 나가고, 또 API가 먹통이면 테스트를 할 수 없어지니깐요.
...
`embedDocuments`는 getVector가 쓰지 않는 메서드여서, 지금 테스트는 안 쓰는 메소드가 안 불렸어? 를 검증하는게 되어버렸습니다.

근데 통과되니깐, 신나서 AI에게 맞게 짰냐고 물어봤는데, 네. 틀렸답니다. 그러면서 기준을 하나 배웠습니다.
...
테스트 코드는 그대로 두고, `QueryVectorService` 쪽만 한 글자 바꿨습니다.
...
제가 잡고 싶었던 바로 그 고장을 손으로 만든 겁니다.<br>
...
if (false)로 바꿨으니 else로 빠져서 embedQuery가 불립니다.
그런데 저는 이 일꾼 한테 일감을 안 줬습니다. never()로 검증할 거니깐 줄 이유가 없었으니깐요.

일감 없는 일꾼한테 일을 시키면 뭘 들고 올까요? 빈손으로 옵니다. null이요.
